            ...
            steps {
                ...
                sh 'docker buildx version' // Docker Buildx가 설치되었는지 확인
                    sh '''
                        ...
                        docker buildx build --progress=plain -t backend:latest --build-arg SPRING_MAIL_USERNAME=${SPRING_MAIL_USERNAME} --build-arg SPRING_MAIL_PASSWORD=${SPRING_MAIL_PASSWORD} --build-arg AWS_ACCESS_KEY_ID=${AWS_ACCESS_KEY_ID} --build-arg AWS_SECRET_ACCESS_KEY=${AWS_SECRET_ACCESS_KEY} -f /var/lib/jenkins/jobs/backend-pipeline/workspace/Dockerfile /var/lib/jenkins/jobs/backend-pipeline/workspace
                    '''
